...
public class HUD
    ...
    private final Setting<String> customWatermark = this.register(new Setting<String>("WatermarkName", "ssk.club b1"));
    ...
    public void onRender2D(final Render2DEvent event) {
        ...
        switch (this.watermark.getValue()) {
            case PHOBOS: {
                this.renderer.drawString("phoBOSS" + (this.modeVer.getValue() ? " v1.9.1" : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2) : this.color, true);
                break;
            }
            case REMAS: {
                this.renderer.drawString("PhoBOSS Rewrite by SSK" + (this.modeVer.getValue() ? " v1.9.1" : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2) : this.color, true);
                break;
            }
            case CUSTOM: {
                this.renderer.drawString(this.customWatermark.getValue() + (this.modeVer.getValue() ? " v1.9.1" : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2) : this.color, true);
                break;
            }
        }
    ...
    public void renderGreeter() {
        ...
        switch (this.greeter.getValue()) {
            ...
            case LONG: {
                text = text + "Welcome to Phobos 1.9.1 Remastered" + HUD.mc.player.getDisplayNameString() + " :^)";
                break;
            }
            case CUSTOM: {
            ...
            default: {
                text = text + "yo wsp " + HUD.mc.player.getDisplayNameString();
                break;
            }
        }
