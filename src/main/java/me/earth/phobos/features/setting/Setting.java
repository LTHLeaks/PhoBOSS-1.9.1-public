...
class Setting< T > {
    ...
    String currentEnumName ( ) {
        return EnumConverter.getProperName ( (Enum < me.earth.phobos.features.modules.combat.Offhand.Mode2 >) this.value );
    }

    public
    int currentEnum ( ) {
        return EnumConverter.currentEnum ( (Enum < me.earth.phobos.features.modules.combat.Offhand.Mode2 >) this.value );
    }

    public
    void increaseEnum ( ) {
        this.plannedValue = (T) EnumConverter.increaseEnum ( (Enum < me.earth.phobos.features.modules.combat.Offhand.Mode2 >) this.value );
        ClientEvent event = new ClientEvent ( this );
        MinecraftForge.EVENT_BUS.post ( event );
        if ( ! event.isCanceled ( ) ) {
            this.value = this.plannedValue;
    ...
    void increaseEnumNoEvent ( ) {
        this.value = (T) EnumConverter.increaseEnum ( (Enum < me.earth.phobos.features.modules.combat.Offhand.Mode2 >) this.value );
    }
